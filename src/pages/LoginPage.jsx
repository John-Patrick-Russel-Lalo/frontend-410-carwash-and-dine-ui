...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // If rememberMe is FALSE → clear token on refresh
      if (!rememberMe) {
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gray-900 hover:bg-gray-700 text-white font-medium py-3 px-4 rounded-lg transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isLoading ? (
